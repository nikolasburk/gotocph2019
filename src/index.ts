...
const Query = queryType({
  definition(t) {
    ...
    t.field('user', {
      ...
      nullable: true,
      args: { id: idArg() },
      resolve: (_, args) => photon.users.findOne({
        where: { id: args.id }
      }),
    })
...
const Mutation = mutationType({
  definition(t) {
    t.field('createUser', {
      ...
      resolve: (_, args) => photon.users.create({
        data: {
          name: args.name,
          email: args.email
        }
    ...
    t.field('updateUser', {
      ...
      resolve: (_, args) => photon.users.update({
        where: { id: args.id },
        data: {
          name: args.name,
          email: args.email
    ...
    t.field('createDraft', {
      ...
      resolve: (_, args) => photon.posts.create({
        data: {
          title: args.title,
